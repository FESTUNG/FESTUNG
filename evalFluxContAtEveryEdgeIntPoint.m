...
warning('ich muss in evalFluxContAtEveryEdgeIntPoint die Zeit fuer die Flussauswertung verwenden!');
...
% %             dataEval(:, :, iT, iE)  = ~g.markE0Tint(iT, iE) * fluxCont( F1(x1, x2), F2(x1, x2), cCont(edgeNr, :) );
% %             dataEval( iT, iE, :, :) = ~g.markE0Tint(iT, iE) * fluxCont( F1(x1, x2), F2(x1, x2), cCont(edgeNr, :) );
%             dataEval( iT, iE, :, :) = ~g.markE0Tint(iT, iE) * fluxCont( F1(x1, x2), F2(x1, x2), cCont(edgeNr, :) );
%         else
...
%     problemData.u1Cont( 0, F1(Q1, Q2), F2(Q1, Q2) 
...
%     dataEvalTest(:, n, 1, :) = kron( ones(2,2), Kkn ) .* problemData.u1Cont( 0, F1(Q1, Q2), F2(Q1, Q2) ) .* cCont(:, n, :)  ;
%   dataEvalTest(:, n, 2, :) = kron( ones(2,2), Kkn ) .* problemData.u2Cont( 0, F1(Q1, Q2), F2(Q1, Q2) ) .* cCont(:, n, :)  ;
% 

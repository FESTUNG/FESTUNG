...
% with a discrete 1D function, with the normal "vector" and divided by the 
...
%>        functions with a discrete 1D function, with the normal "vector" and 
%>        divided by the smoothed mesh height.
%===============================================================================
%>
%> @brief Assembles a matrix containing products of two one-dimensional basis 
%>        functions with a discrete 1D function, with the normal "vector" and 
%>        divided by the smoothed mesh height.
%>
%> The matrix @f$\overline{\mathsf{P}} \in 
...
%> @f[
%> \left[\overline{\mathsf{P}}\right]_{(\overline{k}-1)\overline{N}+i,
...
%> \left[\overline{\mathsf{P}}\right]_{(\overline{k}^--1)\overline{N}+i,
%>     (\overline{k}^+-1)\overline{N}+j} :=
%>   \frac{1}{2} \sum_{a^1_{\overline{k}^-n^-}\in\partial \overline{T}_{\overline{k}}
%>     \cap \overline{\mathcal{V}}_\Omega}
%> \frac{\nu_{\overline{k}^-\overline{n}^-}}{H_s\left(a^1_{\overline{k}\overline{n}^-}\right)}
%> \sum_{l=1}^{\overline{N}} \overline{U}_{\overline{k}^+l}\left(a^1_{\overline{k}\overline{n}}\right)\;
%> \phi_{\overline{k}^-i} \left(a^1_{\overline{k}^-\overline{n}^-}\right) \, 
%> \phi_{\overline{k}^+l}\left(a^1_{\overline{k}^-\overline{n}^-}\right) \, 
%> \phi_{\overline{k}^+j}\left(a^1_{\overline{k}^-\overline{n}^-}\right) 
...
%>     \cap \overline{\mathcal{V}}_\Omega}
%> \frac{\nu_{\overline{k}^-\overline{n}^-}}{H_s\left(a^1_{\overline{k}\overline{n}^-}\right)}
%> \sum_{l=1}^{\overline{N}} \overline{U}^s_{\overline{k}^+l}  
%>  \left[\hat{\bar{\mathsf{P}}}^{s,\text{offdiag}}\right]_{i,j,l,\overline{n}} \,,
%> @f]
...
%> @param refEdgePhiIntPhiExtPhiExt Local matrices 
%>                    @f$\hat{\bar{\mathsf{P}}}^\text{offdiag}@f$ as provided
%>                    by <code>computePhiIntPhiExtPhiExtV0T1D()</code>.
%>                    @f$[2 \times 1 \text{ cell}]@f$
%>

...
% functions with a (spatial) derivative of a 
% discontinuous coefficient function.
%
%===============================================================================
%> @file assembleMatElemPhiPhiDfuncDiscLin.m
%>
...
%> @f[
%>  \mathsf{G}_{T_k}^1 = \sum_{l=1}^N {zb}_{kl} \left(
%>    B_k^{22}[\hat{\mathsf{G}}]_{:,:,l,1} 
%>    - B_k^{21}[\hat{\mathsf{G}}]_{:,:,l,2} \right)
%> @f]
%> and
%> @f[
%>  \mathsf{G}_{T_k}^2 = \sum_{l=1}^N {zb}_{kl} \left(
...
%> @param refElemPhiPhiDphiLin Local matrix @f$\hat{\mathsf{G}}@f$ as provided
...
%> @endparblock
%
function ret = assembleMatElemPhiPhiDfuncDiscLin(g, refElemPhiPhiDphiLin, dataDiscLin)
 K = size(dataDiscLin,1); N = size(refElemPhiPhiDphiLin,2);

% Check function arguments that are directly used
validateattributes(dataDiscLin, {'numeric'}, {'size', [g.numT 3]}, mfilename, 'dataDisc');
validateattributes(refElemPhiPhiDphiLin, {'numeric'}, {'size', [N N 3 2]}, mfilename, 'refElemDphiPhiPhi');

% Assemble matrices
ret = cell(2, 1);
ret{1} = sparse(K*N, K*N);  ret{2} = sparse(K*N, K*N);
for l = 1 : 3
  ret{1} = ret{1} + kron(spdiags(dataDiscLin(:,l) .* g.B(:,2,2), 0, K, K), refElemPhiPhiDphiLin(:,:,l,1)) ...
                  - kron(spdiags(dataDiscLin(:,l) .* g.B(:,2,1), 0, K, K), refElemPhiPhiDphiLin(:,:,l,2));
  ret{2} = ret{2} - kron(spdiags(dataDiscLin(:,l) .* g.B(:,1,2), 0, K, K), refElemPhiPhiDphiLin(:,:,l,1)) ...
                  + kron(spdiags(dataDiscLin(:,l) .* g.B(:,1,1), 0, K, K), refElemPhiPhiDphiLin(:,:,l,2));
end % for
end % function

...
% basis functions and a linear function from the interior of each element with 
% a component of the edge normal.
%
%===============================================================================
%> @file assembleMatEdgePhiIntPhiIntPhiLinNu.m
%>
%> @brief Assembles two matrices containing integrals over edges of products of 
%>        two basis functions and a linear function from the interior of each 
...
%>        and a linear function from the interior of each 
%>        element with a component of the edge normal.
%>
%> The matrix @f$\mathsf{{Q}}^m_\mathrm{N} \in \mathbb{R}^{KN\times KN}@f$
%> is block diagonal and defined as 
%> @f[
%> [\mathsf{{Q}}^m_\mathrm{N}]_{(k-1)N+i,(k-1)N+j} = sum_{l=1}^3 zbDisc(k,l)
...
%> @f[
%> \mathsf{{Q}}^m_\mathrm{N} = \sum_{n=1}^3 \sum_{l=1}^3
%>   \begin{bmatrix}
...
%> @f$\hat{\mathsf{{S}}}^\mathrm{diag}\in\mathbb{R}^{N\times N \times 3\times 3}@f$
%> are given by
%> @f[
...
%> @param refEdgePhiIntPhiIntPhiLin  Local matrix 
...
%> @endparblock
%
function ret = assembleMatEdgePhiIntPhiIntPhiLinNu(g, markE0Tbdr, refEdgePhiIntPhiIntPhiLin, dataDiscLin, areaNuE0Tbdr)
% Extract dimensions
K = g.numT;  N = size(refEdgePhiIntPhiIntPhiLin, 1);

% Check function arguments that are directly used
validateattributes(markE0Tbdr, {'logical'}, {'size', [K 3]}, mfilename, 'markE0Tbdr');
validateattributes(refEdgePhiIntPhiIntPhiLin, {'numeric'}, {'size', [N N 3 3]}, mfilename, 'refEdgePhiIntPhiIntPhiLin');

if nargin > 4
  ret = assembleMatEdgePhiIntPhiIntNu_withAreaNuE0Tbdr(g, refEdgePhiIntPhiIntPhiLin, areaNuE0Tbdr, dataDiscLin);
elseif isfield(g, 'areaNuE0T')
  ret = assembleMatEdgePhiIntPhiIntNu_noAreaNuE0Tbdr_withAreaNuE0T(g, markE0Tbdr, refEdgePhiIntPhiIntPhiLin, dataDiscLin);
else
  ret = assembleMatEdgePhiIntPhiIntNu_noAreaNuE0Tbdr_noAreaNuE0T(g, markE0Tbdr, refEdgePhiIntPhiIntPhiLin, dataDiscLin);
end % if
end % function
%
%===============================================================================
...
%
function ret = assembleMatEdgePhiIntPhiIntNu_withAreaNuE0Tbdr(g, refEdgePhiIntPhiIntPhiLin, areaNuE0Tbdr, dataDiscLin)
% Extract dimensions
K = g.numT;  N = size(refEdgePhiIntPhiIntPhiLin, 1);
...
for n = 1 : 3
  for l = 1 : 3
    ret{1} = ret{1} + kron(spdiags(areaNuE0Tbdr{n,1} .* dataDiscLin(:,l), 0,K,K), refEdgePhiIntPhiIntPhiLin(:,:,l,n));
    ret{2} = ret{2} + kron(spdiags(areaNuE0Tbdr{n,2} .* dataDiscLin(:,l), 0,K,K), refEdgePhiIntPhiIntPhiLin(:,:,l,n));
  end % for
end % for
end % function
%
...
%
function ret = assembleMatEdgePhiIntPhiIntNu_noAreaNuE0Tbdr_withAreaNuE0T(g, markE0Tbdr, refEdgePhiIntPhiIntPhiLin, dataDiscLin)
% Extract dimensions
K = g.numT;  N = size(refEdgePhiIntPhiIntPhiLin, 1);
...
for n = 1 : 3
  for l = 1 : 3
    ret{1} = ret{1} + kron(spdiags(markE0Tbdr(:,n).*g.areaNuE0T{n,1} .* dataDiscLin(:,l), 0,K,K), refEdgePhiIntPhiIntPhiLin(:,:,l,n));
    ret{2} = ret{2} + kron(spdiags(markE0Tbdr(:,n).*g.areaNuE0T{n,2} .* dataDiscLin(:,l), 0,K,K), refEdgePhiIntPhiIntPhiLin(:,:,l,n));
  end % for
end % for
end % function
%
...
%
function ret = assembleMatEdgePhiIntPhiIntNu_noAreaNuE0Tbdr_noAreaNuE0T(g, markE0Tbdr, refEdgePhiIntPhiIntPhiLin, dataDiscLin)
% Extract dimensions
K = g.numT;  N = size(refEdgePhiIntPhiIntPhiLin, 1);
...
for n = 1 : 3
  ...
  for l = 1 : 3
    ret{1} = ret{1} + kron(spdiags(QNkn .* g.nuE0T(:,n,1) .* dataDiscLin(:,l), 0,K,K), refEdgePhiIntPhiIntPhiLin(:,:,l,n));
    ret{2} = ret{2} + kron(spdiags(QNkn .* g.nuE0T(:,n,2) .* dataDiscLin(:,l), 0,K,K), refEdgePhiIntPhiIntPhiLin(:,:,l,n));
  end % for
end % for
end % function

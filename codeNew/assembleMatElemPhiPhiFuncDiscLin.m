...
% functions and a discontinuous coefficient function.
%
%===============================================================================
%> @file assembleMatElemPhiPhiFuncDiscLin.m
%>
...
%>     \varphi_{ki}\varphi_{kl}\varphi_{kj}\mathrm{d}\mathbf{x}\,.
%> @f]
...
%> @param refElemPhiPhiPhiLin Local matrix @f$\hat{\mathsf{D}}@f$ as provided
%>                    by <code>integrateRefElemPhiPhiPhiLin()</code>.
...
%> @endparblock
%
function ret = assembleMatElemPhiPhiFuncDiscLin(g, refElemPhiPhiPhiLin, dataDiscLin)
 K = size(dataDiscLin,1); N = size(refElemPhiPhiPhiLin,1);

% Check function arguments that are directly used
validateattributes(dataDiscLin, {'numeric'}, {'size', [g.numT 3]}, mfilename, 'dataDisc');
validateattributes(refElemPhiPhiPhiLin, {'numeric'}, {'size', [N N 3]}, mfilename, 'refElemPhiPhiPhiLin');

% Assemble matrix
ret = sparse(K*N, K*N);
for l = 1 : 3
  ret = ret + 2 * kron(spdiags(g.areaT .* dataDiscLin(:,l) , 0, K, K), refElemPhiPhiPhiLin(:,:,l));
end % for
end % function

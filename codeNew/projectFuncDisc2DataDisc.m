...
%
%===============================================================================
%> @file projectFuncDisc2DataDisc.m
%>
...
%> @param  funcDisc   A coefficient matrix representing the function @f$d(t)@f$
...
%> @endparblock
%
function dataDisc = projectFuncDisc2DataDisc(funcDisc, ord, refElemPhiPhi)
global gPhi2D
% validateattributes(funcDisc, {'function_handle'}, {}, mfilename, 'funcCont');
ord = max(ord,1); [~, ~, W] = quadRule2D(ord);
N = size(refElemPhiPhi, 1);
rhs = funcDisc * (repmat(W.', 1, N) .* gPhi2D{ord});

...
% consist of all permutations of a two-dimensional and a one-dimensional basis 
% function.
...
%>        integrands consist of all permutations of a two-dimensional and a 
...
%> @endparblock
%
function ret = integrateRefEdgeQuadriPhiIntPhi1DInt(N, qOrd, basesOnQuad2D, basesOnQuad1D)
[~, W] = quadRule1D(qOrd);
ret = zeros(N(1), N(2), 4);
for n = 1 : 2
  for i = 1 : N(1)
...
for n = 3 : 4
  for i = 1 : N(1)
    for j = 1 : N(2)
      ret(i,j,n) = W * ( basesOnQuad2D.phi1D{qOrd}(:,i,n) * basesOnQuad1D.phi0D{qOrd}(j,5-n) );
    end  % for j
  end  % for i
end  % for n
end  % function
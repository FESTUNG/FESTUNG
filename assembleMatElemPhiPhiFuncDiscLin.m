...
%> @endparblock
%
function ret = assembleMatElemPhiPhiFuncDiscLin(g, refElemDphiLinPhiPhi, dataDiscLin)
 K = size(dataDiscLin,1); N = size(refElemDphiLinPhiPhi,2);

% Check function arguments that are directly used
validateattributes(dataDiscLin, {'numeric'}, {'size', [g.numT 3]}, mfilename, 'dataDisc');
validateattributes(refElemDphiLinPhiPhi, {'numeric'}, {'size', [N N 3 2]}, mfilename, 'refElemDphiPhiPhi');

% Assemble matrices
ret = cell(2, 1);
ret{1} = sparse(K*N, K*N);  ret{2} = sparse(K*N, K*N);
for l = 1 : 3
  ret{1} = ret{1} + kron(spdiags(dataDiscLin(:,l) .* g.B(:,2,2), 0, K, K), refElemDphiLinPhiPhi(:,:,l,1)) ...
                  - kron(spdiags(dataDiscLin(:,l) .* g.B(:,2,1), 0, K, K), refElemDphiLinPhiPhi(:,:,l,2));
  ret{2} = ret{2} - kron(spdiags(dataDiscLin(:,l) .* g.B(:,1,2), 0, K, K), refElemDphiLinPhiPhi(:,:,l,1)) ...
                  + kron(spdiags(dataDiscLin(:,l) .* g.B(:,1,1), 0, K, K), refElemDphiLinPhiPhi(:,:,l,2));
end % for
end % function

...
%> @endparblock
%
function ret = assembleMatEdgePhiLagrPhiIntNu(g, markE0Tbdr, refEdgePhiLagrPhiInt, areaNuE0Tbdr)
% Extract dimensions
K = g.numT;  N = size(refEdgePhiLagrPhiInt, 2);

% Check function arguments that are directly used
validateattributes(markE0Tbdr, {'logical'}, {'size', [K 3]}, mfilename, 'markE0Tbdr');
validateattributes(refEdgePhiLagrPhiInt, {'numeric'}, {'size', [3 N 3]}, mfilename, 'refEdgePhiIntPhiInt');

if nargin > 3
  ret = assembleMatEdgePhiIntPhiIntNu_withAreaNuE0Tbdr(g, refEdgePhiLagrPhiInt, areaNuE0Tbdr);
elseif isfield(g, 'areaNuE0T')
  ret = assembleMatEdgePhiIntPhiIntNu_noAreaNuE0Tbdr_withAreaNuE0T(g, markE0Tbdr, refEdgePhiLagrPhiInt);
else
  ret = assembleMatEdgePhiIntPhiIntNu_noAreaNuE0Tbdr_noAreaNuE0T(g, markE0Tbdr, refEdgePhiLagrPhiInt);
end % if
end % function
%
%===============================================================================
...
%
function ret = assembleMatEdgePhiIntPhiIntNu_withAreaNuE0Tbdr(g, refEdgePhiLagrPhiInt, areaNuE0Tbdr)
% Extract dimensions
K = g.numT;  L = g.numV;  N = size(refEdgePhiLagrPhiInt, 2);
...
for n = 1	:	3
	...
	ret{1} = ret{1} + kron(bsxfun(@times, markV0E0T, areaNuE0Tbdr{n,1}'), refEdgePhiLagrPhiInt(mod(n,3)+1,:,n));
  ret{2} = ret{2} + kron(bsxfun(@times, markV0E0T, areaNuE0Tbdr{n,2}'), refEdgePhiLagrPhiInt(mod(n,3)+1,:,n));
  
  markV0E0T = sparse(bsxfun(@eq, (1:L), g.V0T(:,mod(n+1,3)+1)))';
	ret{1} = ret{1} + kron(bsxfun(@times, markV0E0T, areaNuE0Tbdr{n,1}'), refEdgePhiLagrPhiInt(mod(n+1,3)+1,:,n));
  ret{2} = ret{2} + kron(bsxfun(@times, markV0E0T, areaNuE0Tbdr{n,2}'), refEdgePhiLagrPhiInt(mod(n+1,3)+1,:,n));
end % for
end % function
%
%===============================================================================
...
%
function ret = assembleMatEdgePhiIntPhiIntNu_noAreaNuE0Tbdr_withAreaNuE0T(g, markE0Tbdr, refEdgePhiLagrPhiInt)
% Extract dimensions
K = g.numT;  L = g.numV;  N = size(refEdgePhiLagrPhiInt, 2);
...
for n = 1 : 3
  markV0E0T = sparse(bsxfun(@eq, (1:L), g.V0T(:,mod(n,3)+1)))'; 
  ret{1} = ret{1} + kron(bsxfun(@times, markV0E0T, markE0Tbdr(:,n)' .* g.areaNuE0T{n,1}'), refEdgePhiLagrPhiInt(mod(n,3)+1,:,n));
  ret{2} = ret{2} + kron(bsxfun(@times, markV0E0T, markE0Tbdr(:,n)' .* g.areaNuE0T{n,2}'), refEdgePhiLagrPhiInt(mod(n,3)+1,:,n));
  
  markV0E0T = sparse(bsxfun(@eq, (1:L), g.V0T(:,mod(n+1,3)+1)))'; 
  ret{1} = ret{1} + kron(bsxfun(@times, markV0E0T, markE0Tbdr(:,n)' .* g.areaNuE0T{n,1}'), refEdgePhiLagrPhiInt(mod(n+1,3)+1,:,n));
  ret{2} = ret{2} + kron(bsxfun(@times, markV0E0T, markE0Tbdr(:,n)' .* g.areaNuE0T{n,2}'), refEdgePhiLagrPhiInt(mod(n+1,3)+1,:,n));
end % for
end % function
%
%===============================================================================
...
%
function ret = assembleMatEdgePhiIntPhiIntNu_noAreaNuE0Tbdr_noAreaNuE0T(g, markE0Tbdr, refEdgePhiLagrPhiInt)
% Extract dimensions
K = g.numT;  L = g.numV;  N = size(refEdgePhiLagrPhiInt, 2);
...
for n = 1 : 3
  ...
  ret{1} = ret{1} + kron(Qkn1, refEdgePhiLagrPhiInt(mod(n,3)+1,:,n));
  ret{2} = ret{2} + kron(Qkn2, refEdgePhiLagrPhiInt(mod(n,3)+1,:,n));
  ...
  ret{1} = ret{1} + kron(Qkn1, refEdgePhiLagrPhiInt(mod(n+1,3)+1,:,n));
  ret{2} = ret{2} + kron(Qkn2, refEdgePhiLagrPhiInt(mod(n+1,3)+1,:,n));
end % for
end % function

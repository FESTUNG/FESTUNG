...
[~, W] = quadRule1D(qOrd);
ret = { zeros(N,N,3,3,length(W)), zeros(N,N,3,3,length(W)) }; % [N x N x N x 3 x 3]
for nn = 1 : 3 % 3 edges
  for np = 1 : 3
      for i = 1 : N
        for j = 1 : N
          for m = 1 : 2
            ret{m}(i, j, nn, np, :) = basesOnQuad.gradPhi1D{qOrd}(:,i,nn,m) .* basesOnQuad.thetaPhi1D{qOrd}(:,j,nn,np) .* W.';
          end % for
        end % for
      end % for
  end % for

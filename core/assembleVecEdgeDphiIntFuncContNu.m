...
%> @endparblock
%
function ret = assembleVecEdgeDphiIntFuncContNu(g, markE0T, funcCont, N, basesOnQuad, qOrd)
% Extract dimensions
...
for n = 1 : nEdges
  ...
  markCoefE0T = bsxfun(@times, 0.5 * markE0T(:, n) .* g.areaE0T(:, n) ./ g.areaT, squeeze(g.nuE0T(:, n, :)));
  for i = 1 : N
    ret{1}(:, i) = ret{2}(:, i) + markCoefE0T(:, 1) .* (  ...
                       g.B(:, 2, 2) .* ( funcQ0E * (W' .* basesOnQuad.gradPhi1D{qOrd}(:, i, n, 1)) ) ...
                     - g.B(:, 2, 1) .* ( funcQ0E * (W' .* basesOnQuad.gradPhi1D{qOrd}(:, i, n, 2)) ) );
    ret{2}(:, i) = ret{2}(:, i) + markCoefE0T(:, 2) .* ( ...
                     - g.B(:, 1, 2) .* ( funcQ0E * (W' .* basesOnQuad.gradPhi1D{qOrd}(:, i, n, 1)) ) ...
                     + g.B(:, 1, 1) .* ( funcQ0E * (W' .* basesOnQuad.gradPhi1D{qOrd}(:, i, n, 2)) ) );
  end % for
end % for
ret = { reshape(ret{1}.', K*N, 1), reshape(ret{2}.', K*N, 1) };
end % function

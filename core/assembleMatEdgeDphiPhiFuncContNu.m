...
%> @endparblock
%
function ret = assembleMatEdgeDphiPhiFuncContNu(g, markE0Tbdr, refEdgeDphiIntPhiIntPerQuad, refEdgeDphiIntPhiExtPerQuad, funcCont, qOrd)
% Extract dimensions
K = g.numT;
...
validateattributes(refEdgeDphiIntPhiExtPerQuad, {'cell'}, {'numel', 2});
validateattributes(refEdgeDphiIntPhiExtPerQuad{1}, {'numeric'}, {'size', [N N 3 3 R]});
validateattributes(refEdgeDphiIntPhiExtPerQuad{2}, {'numeric'}, {'size', [N N 3 3 R]});
...
for nn = 1 : 3
  ...
  for np = 1 : 3
    ...
    for r = 1 : R
      RknTimesVal{1} = RknTimesVal{1} + kron(Rkn .* valOnQuad(:, r) .* g.nuE0T(:, nn, 1) .* g.B(:, 2, 2), ...
                                             refEdgeDphiIntPhiExtPerQuad{1}(:, :, nn, np, r)) ...
                                      - kron(Rkn .* valOnQuad(:, r) .* g.nuE0T(:, nn, 1) .* g.B(:, 2, 1), ...
                                             refEdgeDphiIntPhiExtPerQuad{2}(:, :, nn, np, r));
      RknTimesVal{2} = RknTimesVal{2} - kron(Rkn .* valOnQuad(:, r) .* g.nuE0T(:, nn, 2) .* g.B(:, 1, 2), ...
                                             refEdgeDphiIntPhiExtPerQuad{1}(:, :, nn, np, r)) ...
                                      + kron(Rkn .* valOnQuad(:, r) .* g.nuE0T(:, nn, 2) .* g.B(:, 1, 1), ...
                                             refEdgeDphiIntPhiExtPerQuad{2}(:, :, nn, np, r));
    end

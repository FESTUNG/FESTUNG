...
qSys = cell(2,1);
for m = 1 : 2
  qSys{m} = problemData.globM \ ( -problemData.globJD{m} - problemData.globLqTerms{m} + ...
            (problemData.globH{m} - problemData.globQ{m}) * cSys{2} );
end % for m
% Water height
hSys = cSys{1} + problemData.tau * ( problemData.barGlobM \ (problemData.globLh - problemData.globLhPterms + problemData.barGlobJh + ...
            (problemData.barGlobG - problemData.barGlobP - problemData.barGlobPbdr) * cSys{1}) );
% Horizontal velocity
cSys{2} = cSys{2} + problemData.tau * ( problemData.globM \ (problemData.globLu - problemData.globLubdrTerms + problemData.globJu + ...
...
% Vertical velocity
cSys{3} = (problemData.globH{2} - problemData.globQup) \ (problemData.globJD{1} - problemData.globLqTerms{1} + problemData.globJh + ...

...
% of two linear Lagrange basis function and a data-dependent coefficient.
%
%===============================================================================
%> @file assembleMatElemDphiLagrDphiLagrVal.m
%>
%> @brief Assembles a matrix containing integrals of products of (spatial) 
%>        derivatives of two linear Lagrange basis function and a coefficient
%>        depending on the gradient of some data.
%===============================================================================
%>
%> @brief Assembles a matrix containing integrals of products of (spatial) 
%>        derivatives of two linear Lagrange basis function and a coefficient
...
%> component-wise by
%> @f[
%>   [\mathsf{A}_L]_{i,j} = \sum_{T_k \in \Tau_h}\int_{T_k} k(\nabla u)
...
%> @f]
%>
%> We use a modification of the assembly in the book by Knabner and Angerman:
...
%> @endparblock
%
function ret = assembleMatElemDphiLagrDphiLagrVal(g, refElemDphiLagrDphiLagr, dataV, diffKoeff)
K = g.numT; L = g.numV;
...
% Assemble matrices
ret = sparse(L, L);  [Q1, Q2] = quadRule2D(1); % TODO
for k = 1 : K
  nablaUk = zeros(2,1);
  for l = 1 : 3
    nablaUk = nablaUk + dataV(g.V0T(k,l)) * 0.5 / g.areaT(k) * [ g.B(k,2,2) * gradPhiLagr(l,1,Q1,Q2) - g.B(k,2,1) * gradPhiLagr(l,2,Q1,Q2); ...
                                                               - g.B(k,1,2) * gradPhiLagr(l,1,Q1,Q2) + g.B(k,1,1) * gradPhiLagr(l,2,Q1,Q2) ]; 
  end % for
  for i = 1 : 3
    for j = 1 : 3
      ret(g.V0T(k,i),g.V0T(k,j)) = ret(g.V0T(k,i),g.V0T(k,j)) + 0.5 / g.areaT(k) * diffKoeff(nablaUk) * ...
                                    ( g.B(k,:,2)*g.B(k,:,2)' * refElemDphiLagrDphiLagr(i,j,1,1) ...

  ...
  for np = 1 : 3
    ...
    switch pd.typeFlux
      case 'Lax-Friedrichs'
        ...
        if pd.isCoupling
          pd.massFluxQ0E0T(:,nn,:) = pd.massFluxQ0E0T(:,nn,:) + 0.5 * permute( reshape( execin('swe/setNaN2Zero', ...
                                      (cQ0E0Tint{2,nn} ./ hQ0E0Tint{nn} + cQ0E0TE0T{2,nn,np} ./ hQ0E0TE0T{nn,np}) .* pd.g.nuQ0E0T{nn,1} + ...
                                      (cQ0E0Tint{3,nn} ./ hQ0E0Tint{nn} + cQ0E0TE0T{3,nn,np} ./ hQ0E0TE0T{nn,np}) .* pd.g.nuQ0E0T{nn,2} ) + ...

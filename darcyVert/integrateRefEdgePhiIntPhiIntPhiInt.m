function [ hatRdiag ] = integrateRefEdgePhiIntPhiIntPhiInt( p , ord , basesOnQuad )
...
for n = 1 : 4  % number of edge
    for s = 1 : numBases  % basis-function of included function D(t)
        for i = 1 : s
            for j = 1 : i
                hatRdiag(i,j,s,n) = W * ( basesOnQuad.gPhi1D(:,i,n) .* basesOnQuad.gPhi1D(:,s,n) .* basesOnQuad.gPhi1D(:,j,n) );

function [step, error, total_iters, f_evals] = ...
  fdkrylov(f0, f, x, params, lmeth, P)
...
if lmeth==1 || lmeth==2  % GMRES or GMRES(m)
  ...
  % for this purpose
  %
  [step, errstep, total_iters]=fdgmres(f0, f, x, gmparms, P);
  kinn=0;
  %
  ...
  while total_iters==lmaxit && ...
      ...
      kinn < restart_limit
    kinn=kinn+1;
    [step, errstep, total_iters]=fdgmres(f0, f, x, gmparms, P, step);
  end
  %total_iters=total_iters+kinn*lmaxit;
  f_evals=total_iters+kinn;
  error = errstep(end);
  total_iters = [total_iters,kinn];
  %
  % Bi-CGSTAB
  %
elseif lmeth==3
  [step, error, total_iters]=fdbicgstab(f0, f, x, gmparms);
  f_evals=2*total_iters;
  %
  % TFQMR
  %
elseif lmeth==4
  [step, error, total_iters]=fdtfqmr(f0, f, x, gmparms);
  f_evals=2*total_iters;
else

...
%> @endparblock
%
function ret = assembleMatEdgeDPhiPhiNuQuad(g, markE0T, refEdgeDPhiIntPhiIntQuad, refEdgeDPhiIntPhiExtQuad, coefE0T, qOrd, funcVal)
...
for r = 1 : R
  for nn = 1 : nEdges
    ...
    ret = ret ...
            + kron(spdiags(funcVal(:,r,nn) .* markCoefE0T .*  (g.B(:,2,2) .* g.nuE0T(:,nn,1) - g.B(:,1,2) .* g.nuE0T(:,nn,2) ) , 0, K, K), refEdgeDPhiIntPhiIntQuad{1}(:,:,nn,r)) ...
            + kron(spdiags(funcVal(:,r,nn) .* markCoefE0T .* (-g.B(:,2,1) .* g.nuE0T(:,nn,1) + g.B(:,1,1) .* g.nuE0T(:,nn,2) ) , 0, K, K), refEdgeDPhiIntPhiIntQuad{2}(:,:,nn,r));
    for np = 1 : nEdges
      if nnz(g.markE0TE0T{nn, np}) > 0
        ret = ret ...
          - kron(bsxfun(@times, g.markE0TE0T{nn, np}, funcVal(:,r,nn) .* markCoefE0T .* ( g.B(:,2,2) .* g.nuE0T(:,nn,1) - g.B(:,1,2) .* g.nuE0T(:,nn,2) )), refEdgeDPhiIntPhiExtQuad{1}(:,:,nn,np,r)) ...
          - kron(bsxfun(@times, g.markE0TE0T{nn, np}, funcVal(:,r,nn) .* markCoefE0T .* (-g.B(:,2,1) .* g.nuE0T(:,nn,1) + g.B(:,1,1) .* g.nuE0T(:,nn,2) )), refEdgeDPhiIntPhiExtQuad{2}(:,:,nn,np,r));
      end
    end % for np
  end % for nn
end % for r

...
%> @endparblock
%
function ret = assembleMatElemDPhiDPhiPerQuad(g, basesOnQuad, K, N, hatQuadDPhiDPhi, funcVal)
...
if nargin < 6
    funcVal = ones(K,size(basesOnQuad.phi2D{qOrd}(:, 1),1));
end %if nargin < 6

% Integrate phi_i * phi_j * function_value with respect to elements
ret = sparse(K*N,K*N);
for r = 1 : R
    ret = ret + kron(spdiags(funcVal(:,r) .* (( g.B(:,1,2) .* g.B(:,1,2) + g.B(:,2,2) .* g.B(:,2,2) ) ./ (2 * g.areaT)), 0, K, K), hatQuadDPhiDPhi(:,:,1,r)) ...
              - kron(spdiags(funcVal(:,r) .* (( g.B(:,1,1) .* g.B(:,1,2) + g.B(:,2,1) .* g.B(:,2,2) ) ./ (2 * g.areaT)), 0, K, K), hatQuadDPhiDPhi(:,:,2,r)) ...
              + kron(spdiags(funcVal(:,r) .* (( g.B(:,1,1) .* g.B(:,1,1) + g.B(:,2,1) .* g.B(:,2,1) ) ./ (2 * g.areaT)), 0, K, K), hatQuadDPhiDPhi(:,:,3,r));
end

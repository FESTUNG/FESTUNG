...
qSys = cell(2,1);
for m = 1 : 2
  qSys{m} = problemData.globM \ ( -problemData.globJu{m} + problemData.globHQ{m} * cSys{2} );
end % for m
% Water height
...
cSys{2} = problemData.omega(nSubStep) * reshape(problemData.cDiscRK{1, 2}.', [], 1) + (1 - problemData.omega(nSubStep)) * ( ...
            cSys{2} + problemData.tau * ( problemData.globLu - problemData.globLzBot + problemData.globM \ ( ...
              -problemData.globKu - problemData.globJ + problemData.tildeGlobHQ * cSys{1} + ...
              problemData.globEP * cSys{2} + problemData.globGR{1} * qSys{1} + problemData.globGR{2} * qSys{2} ) ) );
% Vertical velocity
